...
cchan::~cchan() {
  ...
  if (ch) assert(pthread_cancel(th) == 0);
...
std::string cchan::recv() { return inq.deq(); }
...
void schan::done(pthread_t myth) {
  ...
  assert(ch);
  tcpchans.erase(iter);
  if (waiting) assert(pthread_cond_signal(&tcpchans_c) == 0);
  assert(pthread_mutex_unlock(&tcpchans_m) == 0);
  delete ch;
}
...
void tcpchan::send(std::string pdu) { outq.enq(pdu); }
...
std::string tcpchan::recv() {
  ...
  while (1) {
    if (xread(&len, sizeof(len)) != 0) return "";
    len = ntohl(len);
    if (len > 10 * 1024 * 1024) {
