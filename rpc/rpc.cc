...
void add_timeout(struct timeval a, int b, struct timespec *result,
...
int rpcc::call1(unsigned int proc, const marshall &req, unmarshall &rep,
                TO to) {
  assert(pthread_mutex_lock(&m) == 0);
...
void rpcc::clock_loop() {
  ...
  while (1) {
    ...
    if (_next_timeout.tv_sec == 0) {
      ...
      signalled = (pthread_cond_timedwait(&_timeout_cond, &_timeout_lock,
    ...
    if (!signalled) _next_timeout.tv_sec = 0;
    assert(pthread_mutex_unlock(&_timeout_lock) == 0);
